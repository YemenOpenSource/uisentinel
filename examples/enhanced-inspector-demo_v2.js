...
async function runEnhancedInspectorDemo() {
  const sentinel = new UISentinel({
    headless: true,
    output: {
      directory: './r3-enhanced-inspector-output',
    },
  });

  const outputDir = path.join(__dirname, '..', 'r3-enhanced-inspector-output');
  ...
  try {
    ...
    // Create a page
    const page = await engine.createPage(`http://localhost:5080/`, 'desktop');
  ...
  console.log('Test 1: Comprehensive element inspection (testimonial-card)');
  console.log('-'.repeat(50));
  const h1Result = await inspector.inspectWithAction(page, '.testimonial-card:first-child', {type: 'hover'}, {
    showOverlay: true,
    showInfo: true,
    ...
    autoSave: true,
    outputName: 'testimonial-card-inspection'
  });
  ...
  console.log('-'.repeat(50));
  const deepResult = await inspector.inspect(page, 'body > section.testimonials-section.section-padding > div > div.testimonials-grid > div:nth-child(1)', {
    showOverlay: true,
    captureScreenshot: true,
    captureElementScreenshot: true,
    includeParent: true,
