...
export class ElementInspector extends BaseExtension {
        ...
        try {
          ...
          const annotationConfig: AnnotationConfig = {
            element: {
              ...
              rect: result.element.viewportPosition || result.element.rect,
  ...
  async inspectMultiple(page: Page, selectors: string[], options: {
    ...
    const { outputName = 'multi-element-inspection', captureScreenshots = true } = options;
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  ...
  /**
   * Inspect with interaction (click, hover, etc.) and capture before/after
   */
